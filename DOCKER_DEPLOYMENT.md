...
| ----------------- | ---- | ------------------------------------------ | ------------------------------------- |
| **gmp-sqlserver** | 1435 | mcr.microsoft.com/mssql/server:2022-latest | Database SQL Server                   |
...
- Start SQL Server container (port 1435)
...
- **Server:** `localhost,1435` (từ host) hoặc `gmp-sqlserver,1433` (từ container)
- **Database:** `GMP_WHO_DB`
- **Username:** `sa`
...
- Kiểm tra port 1435 đã free chưa: `netstat -tlnp | grep 1435`
...
- Đây là môi trường development. Thay đổi passwords trong production:
  ...
  - Jwt\_\_Key trong backend environment
  - Dùng HTTPS trong production
- Database không exposed trực tiếp ra internet (chỉ port 1435 trên localhost)
...
| 1435 | gmp-sqlserver | SQL Server TCP                 |
