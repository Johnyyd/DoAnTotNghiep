...
- **Cách B (Thủ công)**: Chạy lần lượt các file trong thư mục `DATABASE/` theo thứ tự:
  1. `MasterData.sql` (Đơn vị tính, Vật tư, Máy móc)
  2. `UserManagement.sql` (Người dùng)
  3. `UomConversion.sql` (Quy đổi đơn vị)
  4. `ProcessDefinition.sql` (Công thức, Đơn pha chế)
  5. `ProductionExecution.sql` (Lệnh sản xuất, Lô sản xuất)
  6. `InventoryTraceability.sql` (Tồn kho, Truy xuất)
  7. `SystemAudit.sql` & `AuditTrail.sql` (Nhật ký hệ thống)
  8. `AdvancedLogic.sql` & `Immutability.sql` (Logic nâng cao & Bảo mật dữ liệu)
...
- **Dữ liệu Master**: Các bản ghi trong `MasterData.sql` là nền tảng, không nên xóa tùy tiện khi hệ thống đang vận hành.
