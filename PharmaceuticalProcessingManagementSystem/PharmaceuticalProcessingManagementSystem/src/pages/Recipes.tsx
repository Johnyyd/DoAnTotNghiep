...
function normalizeBom(item: any): UiBom {
  return {
    ...
    materialId: Number(item.materialId ?? item.MaterialId ?? 0),
    materialName:
      item.material?.materialName ?? item.Material?.MaterialName ?? "Nguyn liu",
...
function isPackagingMaterial(material?: {
  materialCode?: string;
  materialName?: string;
  type?: string;
}) {
...
function normalizeRouting(item: any): UiRouting {
  return {
    ...
    defaultEquipmentId:
      Number(item.defaultEquipmentId ?? item.DefaultEquipmentId ?? 0) ||
      undefined,
    ...
    areaName:
      ...
      item.Area?.AreaName,
    estimatedTimeMinutes: Number(
      item.estimatedTimeMinutes ?? item.EstimatedTimeMinutes ?? 0,
    ),
    cleanlinessStatus: item.cleanlinessStatus ?? item.CleanlinessStatus,
    standardTemperature:
      item.standardTemperature ?? item.StandardTemperature ?? "",
    ...
    setTemperature:
      item.setTemperature !== undefined && item.setTemperature !== null
        ? Number(item.setTemperature)
        : item.SetTemperature !== undefined
          ? Number(item.SetTemperature)
          : undefined,
    setPressure:
      item.setPressure !== undefined && item.setPressure !== null
        ? Number(item.setPressure)
        : item.SetPressure !== undefined
          ? Number(item.SetPressure)
          : undefined,
    setTimeMinutes:
      item.setTimeMinutes !== undefined && item.setTimeMinutes !== null
        ? Number(item.setTimeMinutes)
        : item.SetTimeMinutes !== undefined
          ? Number(item.SetTimeMinutes)
          : undefined,
...
export default function Recipes() {
  ...
  const [createForm, setCreateForm] = useState<RecipeCreateForm>({
    recipeName: "",
    materialId: 0,
    batchSize: 0,
  });
  ...
  const recipes = useMemo(
    () => toRows<any>(recipesRaw).map(normalizeRecipe),
    [recipesRaw],
  );
  const materials = useMemo<MaterialOption[]>(
    () =>
      toRows<any>(materialsRaw).map((m) => ({
  ...
  const equipments = useMemo<EquipmentOption[]>(
    () =>
      toRows<any>(equipmentsRaw).map((e) => ({
        ...
        technicalSpecification:
          e.technicalSpecification ?? e.TechnicalSpecification ?? "",
      })),
    [equipmentsRaw],
  );

  const finishedMaterials = useMemo(
    () => materials.filter((m) => m.type === "FinishedGood"),
    [materials],
  );
  ...
  const filteredRecipes = useMemo(() => {
    ...
    return recipes.filter(
      (recipe) =>
        (recipe.materialName ?? "").toLowerCase().includes(keyword) ||
        (recipe.recipeName ?? "").toLowerCase().includes(keyword) ||
        String(recipe.recipeId).includes(keyword),
    );
  }, [recipes, search]);
  ...
  const allMaterialById = useMemo(
    () => new Map(materials.map((m) => [m.materialId, m])),
    [materials],
  );
  const selectedMaterialIds = useMemo(
    () => new Set<number>(bomItems.map((i) => i.materialId)),
    [bomItems],
  );
  ...
  const reorderMutation = useMutation({
    mutationFn: (items: { routingId: number; stepNumber: number }[]) =>
      recipesApi.reorderRouting(selectedRecipeId as number, items),
    onSuccess: () =>
  ...
  const addSpecMutation = useMutation({
    mutationFn: (data: any) =>
      recipesApi.addTechSpec(selectedRecipeId as number, data),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["techSpecs", selectedRecipeId],
      });
  ...
  const deleteBomMutation = useMutation({
    mutationFn: (bomId: number) =>
      recipesApi.removeBOMItem(selectedRecipeId as number, bomId),
    onSuccess: async () =>
      queryClient.invalidateQueries({
        queryKey: ["recipeBom", selectedRecipeId],
      }),
  ...
  const addRoutingMutation = useMutation({
    mutationFn: () => {
      const allowedIds = new Set(
        routingSelectableMaterials.map((m) => m.materialId),
      );
      const materialIds = routingForm.materialIds.filter((id) =>
        allowedIds.has(id),
      );
      return recipesApi.addRoutingStep(
        selectedRecipeId as number,
        {
          ...routingForm,
          materialId:
            routingForm.materialId > 0 ? routingForm.materialId : null,
          areaId: routingForm.areaId > 0 ? routingForm.areaId : null,
          defaultEquipmentId:
            routingForm.defaultEquipmentId > 0
              ? routingForm.defaultEquipmentId
              : null,
    ...
    onError: (error: any) => {
      alert(
        error?.response?.data?.message ||
          error?.message ||
          "Cập nhật công thức thất bại. Vui lòng kiểm tra lại.",
      );
    },
  });
  ...
  const updateRoutingMutation = useMutation({
    mutationFn: () => {
      ...
      return recipesApi.updateRoutingStep(
        selectedRecipeId as number,
  ...
  const saveDraftRows = async () => {
    const validRows = bomDraftRows.filter(
      (r) => r.materialId > 0 && r.quantity > 0,
    );
    if (!validRows.length) {
    ...
    for (const row of validRows) {
      if (
        ...
        uniqueRows.has(row.materialId)
      ) {
        alert(
          "Có nguyên liệu bị trùng trong danh sách. Vui lòng chỉ giữ mỗi nguyên liệu 1 dòng.",
        );
        return;
      }
  ...
  const openEditRouting = (item: UiRouting) => {
    ...
    setRoutingForm({
      ...
      standardTemperatureMin: item.standardTemperature
        ? (item.standardTemperature.split("-")[0]?.trim() ?? "")
        : "",
      standardTemperatureMax: item.standardTemperature
        ? (item.standardTemperature.split("-")[1]?.trim() ?? "")
        : "",
      standardHumidityMin: item.standardHumidity
        ? (item.standardHumidity.split("-")[0]?.trim() ?? "")
        : "",
      standardHumidityMax: item.standardHumidity
        ? (item.standardHumidity.split("-")[1]?.trim() ?? "")
        : "",
      standardPressureMin: item.standardPressure
        ? (item.standardPressure.split("-")[0]?.trim() ?? "")
        : "",
      standardPressureMax: item.standardPressure
        ? (item.standardPressure.split("-")[1]?.trim() ?? "")
        : "",
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-neutral-900">
          Quản lý công thức sản xuất dược phẩm
        </h1>
        <p className="text-neutral-500 mt-1">
          Lập định mức nguyên liệu và quy trình công đoạn
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card self-start space-y-3">
          ...
          <div>
            <label className="text-xs text-neutral-500">Tên công thức</label>
            <input
              type="text"
              className="input mt-1"
              placeholder="VD: Công thức thử nghiệm A1"
              value={createForm.recipeName}
              onChange={(e) =>
                setCreateForm({ ...createForm, recipeName: e.target.value })
              }
            />
          </div>
          <div>
          ...
          <button
            ...
            disabled={createForm.materialId <= 0 || createForm.batchSize <= 0}
          >
        ...
        <div className="card self-start">
          ...
          {isLoading ? (
            <p className="text-sm text-neutral-500 italic animate-pulse">
              Đang tải dữ liệu...
            </p>
          ) : isError ? (
            ...
            <div className="space-y-1.5 max-h-[200px] overflow-y-auto pr-1">
              {filteredRecipes.map((recipe) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg border transition text-sm ${selectedRecipeId === recipe.recipeId ? "border-primary-300 bg-primary-50" : "border-neutral-200 hover:border-primary-200"}`}
                >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-semibold text-neutral-900">
                    Lập định mức cho 1 viên thuốc
                  </h3>
                </div>
                <button onClick={addDraftRow} className="btn-secondary text-sm">
                            ...
                            {editing ? (
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  const nextPackaging =
                                    isPackagingMaterial(nextMaterial);
                                  setEditingBomRows((prev) => ({
                                    ...prev,
                                    [item.bomId]: {
                                      ...prev[item.bomId],
                                      ...
                                      quantity: nextPackaging
                                        ? Math.max(
                                            1,
                                            Math.round(
                                              prev[item.bomId].quantity || 1,
                                            ),
                                          )
                                        : prev[item.bomId].quantity,
                                    },
                                  }));
                                }}
                              >
                                {inputMaterials
                                  .filter(
                                    (m) =>
                                      m.materialId === editing.materialId ||
                                      !selectedMaterialIds.has(m.materialId),
                                  )
                                  .map((m) => (
                                    <option
                                      key={m.materialId}
                          ...
                          <td>
                            {editing ? (
                              <input
                                type="number"
                                className="input"
                                disabled={isPackagingMaterial(
                                  materialById.get(editing.materialId),
                                )}
                                value={editing.ratioPercent}
                                onChange={(e) => {
                                  const ratioPercent = Number(e.target.value);
                                  const quantity =
                                    recalcMgFromRatio(ratioPercent);
                                  setEditingBomRows((prev) => ({
                                    ...prev,
                                    [item.bomId]: {
                                      ...prev[item.bomId],
                            ...
                            {editing ? (
                              <div className="flex justify-end gap-2">
                                <button
                                  ...
                                  onClick={() =>
                                    updateBomMutation.mutate(editing)
                                  }
                                >
                                  Lưu
                                </button>
                                <button
                                  ...
                                  onClick={() => cancelEditBom(item.bomId)}
                                >
                                  Huỷ
                                </button>
                              </div>
                              ...
                              <div className="flex justify-end gap-2">
                                ...
                                <button
                                  onClick={() => {
                                    if (confirm("Xoá nguyên liệu này?"))
                                ...
                                prev.map((x, i) =>
                                  i === idx
                                    ? {
                                        ...x,
                                        materialId,
                                        ratioPercent: isPackaging
                                          ? 0
                                          : x.ratioPercent,
                                        quantity: isPackaging
                                          ? Math.max(
                                              1,
              ...
              <div className="flex justify-end">
                <button className="btn-primary" onClick={saveDraftRows}>
                  Lưu định mức
                </button>
              </div>
            </div>
                      ...
                      routingSteps.map((item, idx) => {
                        ...
                        if (item.materialIds) {
                          item.materialIds.split(",").forEach((idStr) => {
                            ...
                            if (mid) {
                              const m = allMaterialById.get(mid);
                              if (m) matNames.push(m.materialName);
                            }
                          });
                        }
                        if (!matNames.length && item.materialName)
                          matNames.push(item.materialName);
                        return (
                          <tr
                            key={item.routingId}
                            draggable
                          ...
                          >
                            ...
                            <td className="text-right">
                              <div className="flex justify-end gap-1">
                                ...
                                <button
                                  onClick={() => {
                                    if (confirm("Xa cng on ny?"))
                                      deleteRoutingMutation.mutate(
                                        item.routingId,
                                      );
                                  }}
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                {techSpecs.length === 0 && (
                  <div className="text-sm text-neutral-500 px-2 py-2 border border-dashed border-neutral-300 rounded-lg">
                    Chưa có tiêu chuẩn kỹ thuật. Bạn có thể thêm mới ngay bên
                    dưới.
                  </div>
                )}
                {techSpecs
                        ...
                        .map((sub) => (
                          ...
                          >
                            ...
                            <button
                              onClick={() => {
                                if (confirm("Xa?"))
                                  deleteSpecMutation.mutate(sub.specId);
                              }}
                              className="opacity-0 group-hover:opacity-100 p-1 text-neutral-400 hover:text-red-500"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                      ...
                      {newSubSpecParent === spec.specId && (
                        <div className="flex items-center gap-2 ml-8 mt-1">
                          <input
                            ...
                            value={newSubSpecContent}
                            onChange={(e) =>
                              setNewSubSpecContent(e.target.value)
                            }
                            onKeyDown={(e) => {
                              if (
                                e.key === "Enter" &&
                                newSubSpecContent.trim()
                              ) {
                                addSpecMutation.mutate({
                                  parentId: spec.specId,
                                  sortOrder: techSpecs.filter(
                                    (s) => s.parentId === spec.specId,
                                  ).length,
                                  content: newSubSpecContent.trim(),
                                  isChecked: false,
                                });
                                setNewSubSpecContent("");
                              }
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder="Nhập tiêu chuẩn kỹ thuật mới..."
                  value={newSpecContent}
                  onChange={(e) => setNewSpecContent(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && newSpecContent.trim()) {
                      addSpecMutation.mutate({
                        parentId: null,
                        sortOrder: techSpecs.filter((s) => !s.parentId).length,
      ...
      {showRoutingModal && (
        ...
        >
          <div
            className="bg-white rounded-2xl w-full max-w-5xl p-6 space-y-4"
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
              ...
              <div className="lg:col-span-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRoutingForm({ ...routingForm, stepName: e.target.value })
                  }
                />
              </div>
              <div className="lg:col-span-3">
                ...
                <div className="flex items-center gap-2 mt-1 mb-2">
                  <button
                    ...
                    onClick={() =>
                      setRoutingForm({
                        ...routingForm,
                        materialIds: routingSelectableMaterials.map(
                          (m) => m.materialId,
                        ),
                        materialId:
                          routingSelectableMaterials[0]?.materialId ?? 0,
                      })
                    }
                  >
                    Chọn tất cả
                ...
                <div className="border border-neutral-200 rounded-lg max-h-[150px] overflow-y-auto p-2 space-y-1">
                  {routingSelectableMaterials.map((m) => (
                    <label
                      key={m.materialId}
                      className="flex items-center gap-2 text-sm cursor-pointer hover:bg-neutral-50 px-1 py-0.5 rounded"
                    >
                      <input
                        type="checkbox"
                        checked={routingForm.materialIds.includes(m.materialId)}
                        onChange={(e) => {
                          const ids = e.target.checked
                            ? [...routingForm.materialIds, m.materialId]
                            : routingForm.materialIds.filter(
                                (id) => id !== m.materialId,
                              );
                          setRoutingForm({
                            ...routingForm,
                  ...
                  {routingSelectableMaterials.length === 0 && (
                    <p className="text-xs text-neutral-500 px-1 py-1">
                      Chưa có nguyên liệu trong phần định mức bên trên.
                    </p>
                  )}
                </div>
              </div>
            ...
            {(() => {
              const currentEquipment = equipments.find(
                (e) => e.equipmentId === routingForm.defaultEquipmentId,
              );
              const eqName = currentEquipment
                ? currentEquipment.equipmentName.toLowerCase()
                : "";
              const isWeighting = eqName.includes("cân điện tử");
              const isPackaging =
                /máy đóng nang|máy lau nang|máy in số lô|máy gấp toa|máy đóng chai/i.test(
                  eqName,
                );
              ...
              return (
                <>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          placeholder="Min"
                          value={routingForm.standardTemperatureMin}
                          onChange={(e) =>
                            setRoutingForm({
                              ...routingForm,
                              standardTemperatureMin: e.target.value,
                            })
                          }
                        />
                        ...
                        <input
                          ...
                          className="input text-center"
                          placeholder="Max"
                          value={routingForm.standardTemperatureMax}
                          onChange={(e) =>
                            setRoutingForm({
                              ...routingForm,
                              standardTemperatureMax: e.target.value,
                            })
                          }
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="input text-center"
                          placeholder="Min"
                          value={routingForm.standardHumidityMin}
                          onChange={(e) =>
                            setRoutingForm({
                              ...routingForm,
                              standardHumidityMin: e.target.value,
                            })
                          }
                        />
                        <span className="text-neutral-400 self-center">-</span>
                        <input
                          ...
                          className="input text-center"
                          placeholder="Max"
                          value={routingForm.standardHumidityMax}
                          onChange={(e) =>
                            setRoutingForm({
                              ...routingForm,
                              standardHumidityMax: e.target.value,
                            })
                          }
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="input text-center"
                          placeholder="Min"
                          value={routingForm.standardPressureMin}
                          onChange={(e) =>
                            setRoutingForm({
                              ...routingForm,
                              standardPressureMin: e.target.value,
                            })
                          }
                        />
                        <span className="text-neutral-400 self-center">-</span>
                        <input
                          ...
                          className="input text-center"
                          placeholder="Max"
                          value={routingForm.standardPressureMax}
                          onChange={(e) =>
                            setRoutingForm({
                              ...routingForm,
                              standardPressureMax: e.target.value,
                            })
                          }
                        />
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="text-xs text-neutral-500">
                        Thời gian cài đặt (phút)
                      </label>
                      <input
                        type="number"
                        className="input disabled:opacity-50 disabled:bg-neutral-100"
                        disabled={disableTime}
                        value={routingForm.setTimeMinutes}
                        onChange={(e) =>
                          setRoutingForm({
                            ...routingForm,
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        className="input disabled:opacity-50 disabled:bg-neutral-100"
                        disabled={disableSetParams}
                        value={routingForm.setTemperature}
                        onChange={(e) =>
                          setRoutingForm({
                            ...routingForm,
