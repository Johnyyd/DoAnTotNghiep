...
Trong ngành công nghiệp Dược phẩm, tiêu chuẩn GMP-WHO (Good Manufacturing Practice - Tiêu chuẩn Thực hành Sản xuất Tốt) là quy chuẩn bắt buộc do Tổ chức Y tế Thế giới ban hành nhằm đảm bảo thuốc được sản xuất một cách đồng nhất và đạt tiêu chuẩn chất lượng khắt khe. Đặc thù cốt lõi của quy trình sản xuất theo GMP là tính "có thể truy xuất" (Traceability) và "không thể chối bỏ" (Non-repudiation) đối với mọi thao tác tác động lên lô sản phẩm.

Tại nhiều nhà máy dược hiện nay, việc quản lý Hồ sơ lô sản xuất (BMR - Batch Manufacturing Record) vẫn dựa chủ yếu vào quy trình giấy tờ thủ công (Paper-based BMR). Điều này gây ra nhiều hạn chế cốt lõi như:

- **Nguy cơ sai sót con người (Human Error):** Nhân sự vận hành có nguy cơ ghi nhận sai lệch số liệu cân đo hoặc thiếu sót trong việc ký xác nhận hoàn tất công đoạn.
- **Tính toán thủ công:** Thao tác tính toán bù trừ khối lượng nguyên liệu thủ công tiềm ẩn rủi ro sai sót, có thể gây thất thoát vật tư hoặc làm sai lệch hàm lượng chuẩn của sản phẩm.
- **Tốn thời gian cho bộ phận QA/QC:** Bộ phận Đảm bảo Chất lượng (QA/QC) tiêu tốn nhiều thời gian cho công tác kiểm tra chéo (double-check) thủ công trên hồ sơ giấy trước khi phê duyệt xuất xưởng lô thuốc.
- **Khó khăn trong truy xuất nguồn gốc:** Khi phát sinh sự cố về nguyên liệu đầu vào, việc tra cứu thủ công từ kho lưu trữ hồ sơ giấy nhằm xác định các lô thành phẩm liên quan là một thách thức lớn về mặt quản trị.

Xuất phát từ thực tiễn trên, việc xây dựng một **Hệ thống Quản lý Quy trình Chế biến Thuốc theo tiêu chuẩn GMP-WHO** dưới dạng điện tử (eBMR - Electronic Batch Manufacturing Record) mang tính cấp thiết. Hệ thống không chỉ giúp số hóa dữ liệu mà còn chủ động kiểm soát sai lệch ngay tại thời điểm thực thi (Real-time deviation control).
...
- **Chuyển đổi số Hồ sơ lô (eBMR):** Số hóa hoàn toàn quy trình chế biến thuốc, cụ thể là quy trình bào chế Viên nang cứng (bao gồm các công đoạn chuẩn bị môi trường, sấy, cân nguyên liệu và trộn bột kép).
- **Tự động hóa kiểm soát:** Xây dựng cơ chế phát hiện sai lệch tự động (ví dụ: phát cảnh báo thời gian thực khi khối lượng cân thực tế vượt biên độ dung sai $\pm 5\%$ so với định mức BOM).
- **Tuân thủ chuẩn mực FDA 21 CFR Part 11:** Đảm bảo mọi thay đổi dữ liệu trong hệ thống đều được lưu vết tự động (Audit Trail) và không thể bị thay đổi trái phép. Đồng thời, hệ thống bắt buộc nhân sự vận hành phải xác thực bằng Chữ ký điện tử (Mã PIN) tại các Điểm kiểm soát tới hạn (Critical Control Points).
...
- **Phạm vi hệ thống:** Đồ án giới hạn trong phạm vi quản lý lệnh sản xuất, theo dõi tiến độ công đoạn, truy xuất nguyên liệu đầu vào, và ghi nhận Audit Trail. Hệ thống chưa tích hợp các phân hệ quản trị nguồn lực doanh nghiệp (ERP) như kế toán, nhân sự hay giá thành.
- **Công nghệ cốt lõi:**
...
Quá trình khảo sát nhằm làm rõ quy trình vận hành thực tế tại phân xưởng, từ đó ánh xạ (mapping) chính xác các tham số môi trường, thời gian thực thi và thiết bị vào mô hình dữ liệu (Data Model) của hệ thống eBMR.

### 1.2. HIỆN TRẠNG TỔ CHỨC VÀ QUY TRÌNH NGHIỆP VỤ

#### 1.2.1. Cấu trúc vai trò (Role-based Structure)

Quy trình sản xuất dược phẩm yêu cầu cơ chế phân quyền nghiêm ngặt nhằm tuân thủ nguyên tắc "bất kiêm nhiệm" (Segregation of Duties) trong GMP:
...
- **Nhân viên vận hành (Operator):** Trực tiếp điều khiển thiết bị tại phân xưởng (cân, sấy, trộn) và ghi nhận kết quả thực tế vào hệ thống.
...
2. **Công đoạn 1 - Sấy nguyên liệu:**
   ...
   - **SOP:** Nhiệt độ sấy thiết lập ở $75^\circ C$, thời gian sấy kéo dài $180$ phút. Nhân viên vận hành phải ghi nhận độ ẩm của hạt sau sấy để đảm bảo đạt tiêu chuẩn (< 5%).
...
3. **Công đoạn 2 - Pha chế (Cân nguyên liệu):**
   ...
   - **Yêu cầu:** Nhân viên vận hành tham chiếu định mức lý thuyết (BOM) trên màn hình Tablet, tiến hành cân, và nhập khối lượng cân thực tế (Actual Weight).

4. **Công đoạn 3 - Trộn khô:**
   - **Thiết bị:** Máy trộn lập phương `AD-LP-200`.
   - **SOP:** Công đoạn này yêu cầu nạp xen kẽ nguyên liệu chính (NLC 3) và các tá dược thành từng lớp mỏng. Thời gian trộn được thiết lập nghiêm ngặt trong $15$ phút với tốc độ $15$ vòng/phút. Mọi sai lệch về thời gian đều có thể gây ra hiện tượng phân lớp khối bột, dẫn đến sự không đồng đều về hàm lượng viên thuốc.
...
- **Mô tả:** Nhân viên vận hành sử dụng ứng dụng di động (Mobile App) để thực thi các công đoạn sấy, cân, trộn theo đúng trình tự được quy định trong Hồ sơ lô.
...
- **Luồng sự kiện chính (Happy Path):**
  1. Nhân sự vận hành chọn Lệnh sản xuất tương ứng trong ca làm việc.
  2. Hệ thống hiển thị công đoạn cần thực thi tiếp theo (ví dụ: Cân nguyên liệu NLC 3).
  3. Nhân sự tiến hành cân và nhập khối lượng thực tế được hiển thị trên cân IW2-60 vào hệ thống.
  4. Hệ thống kiểm tra đối chiếu: Khối lượng thực tế phải nằm trong biên độ $\pm 5\%$ so với định mức (BOM).
  5. Nhân sự xác nhận hoàn tất thao tác, hệ thống yêu cầu cung cấp mã PIN chữ ký số.
  6. Lưu dữ liệu thành công, quy trình tự động chuyển sang công đoạn tiếp theo.
- **Luồng ngoại lệ (Alternative Flow - Xử lý sai lệch):**
  - Tại bước 4, nếu khối lượng nhập vào chênh lệch vượt mức 5%, hệ thống sẽ kích hoạt cảnh báo đỏ.
  - Quy trình thông thường bị chặn. Nhân sự buộc phải chọn chức năng "Báo cáo sai lệch" (Report Deviation) và cung cấp lý do cụ thể.
  - Sau khi ký PIN xác nhận, trạng thái lô chuyển sang `Hold`. Lệnh sản xuất bị tạm ngưng và hệ thống gửi thông báo khẩn cấp đến bộ phận QC.

##### 2. Use-case: Duyệt / Tạm ngưng / Xử lý sai lệch (Quản lý / QC)

- **Mô tả:** QC theo dõi Lệnh sản xuất trên Web Portal. Nếu các thông số kỹ thuật đáp ứng tiêu chuẩn, QC cho phép lệnh tiếp tục. Trường hợp phát hiện sai lệch môi trường hoặc do nhân sự báo cáo lỗi (Hold), QC có trách nhiệm trực tiếp điều tra và xử lý.
- **Luồng xử lý Hold:**
  1. Nhận được thông báo Lô đang ở trạng thái `Hold`.
  2. QC tiến hành kiểm tra thực địa tại phân xưởng (ví dụ: xác định sự cố hỏng cảm biến của cân IW2-60).
  3. QC đưa ra quyết định xử lý trên hệ thống:
     - Chọn `Resume` (Tiếp tục) nếu nguyên nhân sai lệch đã được khắc phục.
     - Chọn `Reject` (Hủy lô) nếu lô bán thành phẩm/thành phẩm không thể cứu vãn và không đạt tiêu chuẩn chất lượng.
  4. Mọi thao tác xử lý trạng thái `Hold` đều bị ghi lưu vết cứng (Hard-Audit) vào cơ sở dữ liệu nhằm phục vụ truy xuất sau này.

### 2.2. Phân tích Use-case hệ thống và Đặc tả

#### 2.2.1. Sơ đồ tuần tự (Sequence Diagram) - Thực thi công đoạn trên Mobile

Nhân sự vận hành thao tác trên ứng dụng Mobile (Flutter) để thực thi công đoạn Cân nguyên liệu. Sơ đồ dưới đây minh họa quá trình tương tác liên tục giữa thiết bị Mobile, Backend API và cơ chế lưu vết tại Database.

```mermaid
sequenceDiagram
    actor Operator as Nhân viên Vận hành (Operator)
...
Lệnh sản xuất trong tiêu chuẩn GMP không cho phép bỏ qua (skip) hoặc lùi lại các công đoạn một cách tự do. Vòng đời của Lệnh sản xuất được biểu diễn nghiêm ngặt bằng sơ đồ trạng thái (State Diagram) như sau:
...
stateDiagram-v2
    ...
    InProcess --> Hold : Hệ thống bắt lỗi / Nhân sự báo sai lệch
    Hold --> InProcess : QC khắc phục sự cố, mở khóa (Resume)
    Hold --> Rejected : Lô bị hủy do không đạt tiêu chuẩn chất lượng
...
- **BatchProcessLog (Nhật ký công đoạn):** Ghi nhận các thao tác của nhân sự tại từng trạm thiết bị (cân IW2-60, máy trộn AD-LP-200) thông qua chữ ký số (Mã PIN), kèm theo các thông số môi trường (nhiệt độ, độ ẩm).
...
2. **Mobile App (Dành cho Công nhân - Flutter):**
   - **Môi trường:** Màn hình cảm ứng (Tablet) đặt tại phòng pha chế, phù hợp với thao tác khi nhân sự đang mang găng tay bảo hộ.
   ...
   - **Màn hình Cảnh báo & Ký số:** Bàn phím số (Numpad) ảo với kích thước lớn được bố trí thuận tiện. Khi phát sinh sai lệch (như sai số cân), hệ thống áp dụng hiệu ứng phản hồi xúc giác (Haptic feedback) kết hợp viền đỏ nhấp nháy nhằm thu hút sự chú ý và cảnh báo kịp thời.
...
Để đáp ứng tiêu chuẩn GMP, các thao tác thay đổi dữ liệu (Create, Update, Delete) phải được giám sát nghiêm ngặt ở mức sâu nhất (Database level). Việc ứng dụng `SaveChangesInterceptor` của Entity Framework là giải pháp tối ưu, loại bỏ hoàn toàn rủi ro thiếu sót từ phía kỹ sư phần mềm trong việc ghi nhận nhật ký hệ thống (log).
...
Trong giao diện eBMR, khi nhân sự vận hành chọn "Ký xác nhận", hệ thống yêu cầu nhập mã PIN định danh gồm 6 chữ số. API `POST /api/batchprocesslogs` sẽ tiếp nhận yêu cầu này.
Tại Backend, hệ thống thực hiện hai lớp xác thực:

1. Xác thực danh tính (Authentication) thông qua JWT Token tại Header.
...
- **Cấu hình Encoding UTF-8:** CSDL được thiết lập chuẩn Collation và mã hóa UTF-8 (Codepage 65001). Cấu hình này đảm bảo các thuật ngữ chuyên ngành tiếng Việt (như "Viên nang", "Cao khô Trinh nữ") được hiển thị chính xác trên mọi nền tảng, khắc phục triệt để tình trạng lỗi bảng mã.
...
- **Mô tả:** Đánh giá luồng dữ liệu thông thường. Quản trị viên lập lệnh sản xuất $\rightarrow$ Quản lý phê duyệt lệnh $\rightarrow$ Nhân sự vận hành thực thi các công đoạn Cân, Sấy, Trộn $\rightarrow$ QC xác nhận hoàn tất.
...
- **Mô tả:** Đánh giá năng lực phát hiện và bẫy lỗi theo thời gian thực. Giả lập trường hợp nhân sự vô tình nhập khối lượng cân thực tế vượt quá biên độ $\pm5\%$ so với định mức lý thuyết (Ví dụ: Định mức yêu cầu 100kg, thực tế nhập 106kg).
...
- **Ưu điểm:** Giao diện Tablet hiển thị trực quan, kích thước thành phần phù hợp cho thao tác chạm. Luồng thực thi dạng Wizard theo từng bước giúp nhân sự tập trung, tránh tình trạng quá tải thông tin.
- **Nhược điểm phát hiện:** Đôi khi nhân sự vận hành quên thao tác đăng xuất, dẫn đến nguy cơ nhân sự ca sau thao tác trên phiên làm việc (session) cũ. Điều này có thể sinh ra các bản ghi rác (như `LoginFailed` do nhập sai mã PIN). Cần tích hợp cơ chế tự động đăng xuất (Auto-logout) sau 15 phút không có tương tác (Idle).
...
- **Chưa tự động hóa thu thập dữ liệu (Manual Data Entry):** Hiện tại, nhân sự vẫn phải đọc số liệu trực quan từ thiết bị cân và nhập thủ công vào Tablet. Bất chấp cơ chế cảnh báo sai lệch, rủi ro nhập sai dữ liệu vẫn hiện hữu.
